...
if [[ $# -eq 0 ]] 
then 
	files=$(find conv_data -regex '.*[bwx]\.bin' | sort --version-sort) 
	...
	then 
		...
		function gemm {
			...
			then 
				...
				input_file=$(find conv_data -regex ".*[^0-9]$1_x\.bin") 
				weight_file=$(find conv_data -regex ".*[^0-9]$1_w\.bin") 
				bias_file=$(find conv_data -regex ".*[^0-9]$1_b\.bin")
				
				# TODO: don't hard-code this in 
				output_1_file="conv_data/conv$1_z1.bin"
				output_2_file="conv_data/conv$1_z2.bin" 
		...
		then 
			...
			then
				...
				echo Missing bin files. Run "./vgg <image.jpg>" to generate more
			fi 
		else 
			# Get the arguments that correspond to the argument 
			# find conv_data -regex '.*5_[bwx]\.bin' 
			gemm $1 
		fi
