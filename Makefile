...
ifeq ("$(wildcard lib/libjpeg/djpeg)", "")
	...
	mkdir -p obj/python 
	mkdir -p bin 
	mkdir -p vgg-conv-data 
	mkdir -p alexnet-conv-data 
...
gemm-compile: pre-build 
	...
	gcc -o obj/c/json.o -c lib/json-parser/json.c $(FLAGS) $(INCLUDES)
	nvcc -o obj/gpu/cudaTensorCoreGemm.o -c src/gpu/cudaTensorCoreGemm.cu $(FLAGS) $(INCLUDES) -arch=sm_70
	nvcc -o bin/gemm obj/gpu/gemm.o obj/c/network.o obj/c/layer.o obj/gpu/layer_gpu.o obj/c/ndarray.o obj/c/image.o obj/c/json.o obj/gpu/cudaTensorCoreGemm.o -lm --cudart shared $(FLAGS)
